...
export async function sendOtpEmail(email: string, code: string): Promise<boolean> {
  ...
  const subject = '🔑 [Pattie Play Shop] Your Registration OTP Verification Code';
  const htmlContent = `
  ...
  // 1. Send via Resend HTTP API (if configured) - Preferred on cloud environments like Render Free Tier
  if (isResendConfigured) {
    try {
      console.log(`[Email Service] Attempting to send email via Resend API to: ${email}`);
      ...
      if (response.ok) {
        ...
        console.log(`[Email Service] Email sent successfully via Resend API. ID: ${resData.id}`);
        return true;
      } else {
        const errText = await response.text();
        console.error(`[Email Service] Resend API failed with status ${response.status}:`, errText);
      }
    } catch (error) {
      console.error(`[Email Service] Failed to send via Resend API:`, error);
    }
  }

  // 2. Fallback to Nodemailer SMTP (if SMTP is configured and Resend failed or is not configured)
  if (isSmtpConfigured) {
    try {
      console.log(`[Email Service] Attempting to send email via SMTP to: ${email}`);
      ...
      console.log(`[Email Service] Email OTP sent successfully via SMTP. Message ID: ${info.messageId}`);
      return true;
    } catch (error) {
