...
export default function OrderDetailClient({ id }: { id: string }) {
  ...
  const handleUploadSubmit = async () => {
    ...
    try {
      await api.upload(`/orders/${id}/upload-slip`, formData);
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-white dark:bg-slate-800/80 border border-[var(--border-color)] shadow-sm print:hidden flex flex-col md:flex-row gap-8 items-center animate-fadeIn">
          ...
          <div className="flex-1 flex flex-col gap-5 w-full">
            ...
            <div className="p-4 rounded-2xl bg-brand-pink-50/50 dark:bg-brand-pink-950/10 border border-brand-pink-100/30 flex items-center justify-between">
              <div>
                <span className="text-[10px] font-bold text-slate-450 uppercase tracking-wider block">{language === 'TH' ? 'ยอดเงินที่ต้องโอน' : language === 'LA' ? 'ຍອດເງິນທີ່ຕ້ອງໂອນ' : 'Amount to Transfer'}</span>
            ...
            {/* Slip Upload Status */}
            {uploadError && (
              <div className="text-xs text-red-500 bg-red-50 dark:bg-red-955/20 border border-red-150 p-3 rounded-2xl flex items-start gap-1.5">
              ...
              <div className="w-full flex flex-col gap-3">
                {selectedFile ? (
                  <div className="p-4 rounded-2xl border border-brand-pink-200 bg-brand-pink-50/5 dark:bg-brand-pink-950/5 text-xs flex flex-col gap-3">
                    <div className="flex items-center justify-between gap-2 text-slate-700 dark:text-slate-355">
